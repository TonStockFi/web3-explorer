...
export default function BubbleSheetGradingPaper() {
    //@ts-ignore
    const { cv } = window;
    ...
    const printAnswersToTable = (answers: any, options = ['A', 'B', 'C', 'D', 'E']) => {
    ...
    const fourPointTransform = (srcMat: any) => {
        let contMat = srcMat.clone();
        let contours = new cv.MatVector();
        let hierarchy = new cv.Mat();
        cv.findContours(contMat, contours, hierarchy, cv.RETR_LIST, cv.CHAIN_APPROX_SIMPLE);
        ...
        sortableContours = sortableContours
            .sort((item1, item2) =>
                item1.areaSize > item2.areaSize ? -1 : item1.areaSize < item2.areaSize ? 1 : 0
            )
            .slice(0, 5);
        ...
        cornerArray
            ...
            .slice(0, 5);
        let tl =
            cornerArray[0].corner.x < cornerArray[1].corner.x ? cornerArray[0] : cornerArray[1];
        let tr =
            cornerArray[0].corner.x > cornerArray[1].corner.x ? cornerArray[0] : cornerArray[1];
        let bl =
            cornerArray[2].corner.x < cornerArray[3].corner.x ? cornerArray[2] : cornerArray[3];
        let br =
            cornerArray[2].corner.x > cornerArray[3].corner.x ? cornerArray[2] : cornerArray[3];
    ...
    const applyOtsuThresh = (src: any) => {
        let thresh = new cv.Mat();
        cv.threshold(src, thresh, 0, 255, cv.THRESH_OTSU);
        return thresh;
    };

    const detectCircles = (mat: any) => {
        let src = mat.clone();
        ...
        let bubbleOutlines = potentialBubbles.reduce((collection, current) => {
            ...
            if (!overlapping) {
                ...
                if (currentArea > otherArea) {
                    let otherIndex = collection.indexOf(overlapping);
    ...
    const sortContours = (contours: any) => {
        const BUFFER = 1.2;
        return contours
            .map((c: any) => [c, cv.boundingRect(c)])
            .sort((a, b) => {
                const [, aRect] = a;
                const [, bRect] = b;
    ...
    const gradeButtonClick = function () {
    ...
    useEffect(() => {
        ...
        let inputCanvas = canvasView.current;
        function loadImageToCanvas() {
            ...
            img.src = 'bubble_sheet_grading_paper/main.png';
        }
        loadImageToCanvas();
    ...
    return (
        <View>
            <View mb12>
                <canvas ref={canvasView} id="canvasInput"></canvas>
            </View>

            <Button variant="contained" onClick={gradeButtonClick}>
                GRADE
            </Button>
            <View mt12 mb12>
