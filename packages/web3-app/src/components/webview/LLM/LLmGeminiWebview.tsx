...
import { onPromptGemini } from '../SideWeb/SideWebviewInner';
import { LLmWebview } from './LLmWebview';

export function LLmGeminiWebview({ tabId }: { tabId: string }) {
    const url = 'https://gemini.google.com/app';
    const [reply, setReply] = useState('');
    const { currentAccount } = usePlayground();

    const pid = `p_llm_${currentAccount!.id}_${currentAccount!.index || 0}`;

    useOnce(() => {
        const ls = new LLMGeminiService(tabId);
        ls.setGeminiIsReady(false);
        ls.getAll().then(rows => {
    ...
    const checkGeminiCb = async () => {
        await new LLMGeminiService(tabId).checkGeminiWebviewIsReady();
    };

    const pullMessageCb = async () => {
        const ls = new LLMGeminiService(tabId);
        if (!ls.getGeminiIsReady()) {
            return;
        }
        ...
        if (message) {
            ...
            await onPromptGemini(tabId, prompt, imageUrl);
        ...
        <View wh100p relative>
            ...
            <View abs left0 right0 top={0} bottom={0} flx>
                <View flex1 h100p borderRadius={2} borderBox overflowHidden relative>
                    <LLmWebview pid={pid} tabId={tabId} url={url} />
                </View>
            </View>
            <ControlsView findInPageTop={72} tabId={tabId} />
        </View>
    );
}
