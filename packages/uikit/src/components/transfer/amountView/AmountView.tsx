...
export const AmountView: FC<{
    ...
    useEffect(() => {
        if (defaults?.inputValue) {
            onInput(defaults?.inputValue);
        }
    }, [defaults]);
    useEffect(() => {
    ...
    const handleSubmit = useCallback(() => {
        if (isValid) {
            if (defaults?.inputValue) {
                onConfirm({ ...amountState, ...defaults });
            } else {
            ...
            {!shouldHideHeaderAndFooter && (
                <NotificationHeaderPortal>
                    <NotificationHeader>
                        <HeaderBlock
                            onClose={onClose}
                            onBack={defaults?.inputValue ? undefined : handleBack}
                        >
                            <SubTitle>
                            ...
                            <Remaining>
                                {t('send_screen_steps_amount_remaining').replace(
                                    ...
                                    formatter.format(remaining, {
                                        decimals: amountState.token.decimals
                                    })
                                )}{' '}
                ...
                <>
                    ...
                    <MaxRow>
                        ...
                        {!enoughBalance && (
                            ...
                            >
                                {t('wallet_receive')}{' '}
                                {amountState.inFiat ? fiat : amountState.token.symbol}
                            </div>
                        )}
                    </MaxRow>
                </>
