...
/**
 * Encrypts plaintext buffer using AES-GCM.
 * @param plaintext {Buffer}
 * @param password {string}
 * @return {Promise<string>}
 */
export async function aesGcmEncryptBuffer(plaintext: Buffer, password: string) {
    const pwUtf8 = new TextEncoder().encode(password); // encode password as UTF-8
    const pwHash = await crypto.subtle.digest('SHA-256', pwUtf8); // hash the password
    ...
    const ptUint8 = new Uint8Array(plaintext); // ensure plaintext is Uint8Array
    const ctBuffer = await crypto.subtle.encrypt(alg, key, ptUint8); // encrypt plaintext using key
...
/**
 * Decrypts ciphertext to a buffer using AES-GCM.
 * @param ciphertext {string}
 * @param password {string}
 * @return {Promise<Buffer>}
 */
export async function aesGcmDecryptToBuffer(ciphertext: string, password: string) {
    const pwUtf8 = new TextEncoder().encode(password); // encode password as UTF-8
    const pwHash = await crypto.subtle.digest('SHA-256', pwUtf8); // hash the password
    ...
    return Buffer.from(plainBuffer); // return the plaintext as a Buffer
}
